...
class CreatedViewController: UIViewController {
	...
	func takeNation() {
		refDatabase.child("UserProfile").child(uid!).observeSingleEvent(of: .value, with: { snapshot in
			...
			self.user.type = dic["Type"] as? String
			self.nation = dic["Location"] as? String
			
			self.refStorage.child("ProfileImage").child(self.uid!).getData(maxSize: 1*1024*1024){
				data, error in
				...
				self.loadCreatedChallenge(nation: dic["Location"] as! String)
			}
	...
	func loadCreatedChallenge(nation: String) {
		
		refDatabase.child("Challenge").child(nation).queryOrdered(byChild: "uidCreator").queryEqual(toValue: uid!).observeSingleEvent(of: .value, with: { snap in
			...
			if snap.childrenCount == 0{
				...
				for child in snap.children {
					...
					self.refStorage.child("Challenge").child(nation).child(snap2.key).getData(maxSize: 1*1024*1024){
						data, error in
						...
						self.refDatabase.child("Challenge").child(nation).child(snap2.key).child("Users").observeSingleEvent(of: .value, with: { snap in
							...
							challenge.typeUser = self.user.type!
...
extension CreatedViewController: UITableViewDelegate, UITableViewDataSource {
	func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		return challengeItems.count
	}
	
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: "created") as! CreatedTableViewCell
		...
		cell.descritionLabel.text = challengeItems[indexPath.row].descriptionAction
        ...
        cell.profileImage.image = challengeItems[indexPath.row].profileImg
...
extension CreatedViewController {
